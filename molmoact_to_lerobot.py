...
Usage example:
    ...
    python molmoact_to_lerobot.py \
        --data_dir /home/nvidia/repo/rby1-examples-vr-teleop/data/close_the_oven \
        --output_dir /home/nvidia/repo/rby1-examples-vr-teleop/data/close_the_oven_lerobot_v30 \
        --repo_id williamtsai726/close_the_oven_0326 \
        --fps 30
...
You can then train with:
    ...
    python src/lerobot/scripts/lerobot_train.py \
        ...
        python src/lerobot/scripts/lerobot_train.py \
            --dataset.repo_id=williamtsai726/close_the_oven_0326     \
            --policy.type=diffusion \
            --policy.repo_id=williamtsai726/close_the_oven_0326 \
            --policy.device=cuda \ 
...
# 16 DOF total: 7 arm joints + 1 gripper per side (matches collect.py extract_left_right_joint_payload).
STATE_DIM_NAMES = [
    "left_joint1",
    "left_joint2",
    "left_joint3",
...
def load_molmoact_data(data_dir: str) -> List[Dict[str, Any]]:
    ...
    for ep_dir in episode_dirs:
        ...
        if has_next_joint:
            ...
            next_qpos = np.concatenate([next_left_joint, next_right_joint], axis=1)  # (T, 16)

        qpos = np.concatenate([left_joint, right_joint], axis=1)   # (T, 16)
        expected_dof = len(STATE_DIM_NAMES)
        if qpos.shape[1] != expected_dof:
            raise ValueError(
                f"Episode {episode_id}: concatenated left+right joint dim is {qpos.shape[1]}, "
                f"expected {expected_dof}. Check left_joint/right_joint length in JSON."
            )
...
def cleanup_stale_images_dir(output_dir: str) -> None:
    """Remove leftover ``images/`` under the dataset root after video export.

    For visual features with ``dtype: video``, LeRobot still writes frames as PNGs
    under ``images/<feature_key>/episode-.../`` while encoding MP4s into
    ``videos/``. The writer's buffer cleanup uses ``image_keys``, which is only
    populated for ``dtype: image``, so the temporary ``images/`` tree is not
    always removed. Training and the v3 layout use ``videos/`` only; Hub upload
    also ignores ``images/`` (see ``LeRobotDataset.push_to_hub``).
    """
    img_root = Path(output_dir) / "images"
    if not img_root.is_dir():
        return
    shutil.rmtree(img_root)
    print(f"Removed temporary images directory: {img_root}")
...
def create_lerobot_dataset_v30(
    ...
    dataset.finalize()
    cleanup_stale_images_dir(output_dir)
